...
import "./vendor/ERC20/ERC20Mintable.sol";
import './Math.sol';
...
contract ConvictionVoting {
    ...
    struct Proposal {
        address payable milestoneAddress;
    ...
    function isCommons() pure public returns (bool) {
        return true;
    }
    ...
    //TODO: Test with real function (calculateAndSetConviction)
    function fakeProposalPass(uint id) public {
        Proposal storage proposal = proposals[id];
        emit ProposalPassed(id, 10000000000);
        address(proposal.milestoneAddress).call.value(proposal.amount_commons)("");
    }
    ...
    function calculateAndSetConviction(uint256 id, uint256 old_staked) internal {
        ...
        // Get current askAmount
        uint amount = getProposalAmountRequired(id);
        if (conviction > calculateThreshold(amount)) {
            emit ProposalPassed(id, conviction);
            address(proposal.milestoneAddress).call.value(amount)("");
        }
    }

    function getProposalAmountRequired(uint proposalId) internal view returns (uint) {
        Milestone milestone = Milestone(proposals[proposalId].milestoneAddress);
        return milestone.getAskAmount();
    }
